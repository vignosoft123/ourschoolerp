...
class WhatsappTemplate extends Admin_Controller {
/*
| -----------------------------------------------------
...
*/
	...
	public function index() {
		...
		$this->data["subview"] = "whatsapptemplate/index";
		...
		if($_POST) {
			...
			if($type == 'email') {
				...
				if ($this->form_validation->run() == FALSE) {
					...
					$this->data["subview"] = "whatsapptemplate/add";
					$this->load->view('_layout_main', $this->data);
				} else {
					$array = array(
					...
					redirect(base_url('whatsapptemplate/index'));
				}
			} elseif($type == "sms") {
				$this->data["email"] = 0;
				$this->data["sms"] = 1;
			...
			if($this->data['mailandsmstemplate']) {
				...
				if($_POST) {
					if($this->data['mailandsmstemplate']->type == 'email') {
						...
						$this->form_validation->set_rules($rules);
						if ($this->form_validation->run() == FALSE) {
							$this->data["subview"] = "whatsapptemplate/edit";
							$this->load->view('_layout_main', $this->data);
						} else {
							$array = array(
								'name' => $this->input->post('email_name'),
	...
	public function view() {
		...
		if((int)$id) {
			$this->data['mailandsmstemplate'] = $this->mailandsmstemplate_m->get_mailandsmstemplate($id);
			if($this->data['mailandsmstemplate']) {
				$this->data["subview"] = "whatsapptemplate/view";
				$this->load->view('_layout_main', $this->data);
			} else {
				$this->data["subview"] = "error";
				$this->load->view('_layout_main', $this->data);
