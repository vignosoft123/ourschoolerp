...
class Admitcardreport extends Admin_Controller {
	...
	private function getQuery(&$queryArray,$posts){
		...
		$sectionID = $posts['sectionID'];
		$schoolyearID = $this->session->userdata('defaultschoolyearID');
		$queryArray['examschedule.schoolyearID'] = $schoolyearID;

		if((int)$examID > 0) { 
			$queryArray['examschedule.examID'] = $examID;
		}
		if((int)$classesID > 0) {
			$queryArray['examschedule.classesID'] = $classesID;
		}
		if((int)$sectionID > 0) {
			$queryArray['examschedule.sectionID'] = $sectionID;
		}
	}
		...
		if(permissionChecker('admitcardreport')) {
			...
			if($_POST) {
				...
				if ($this->form_validation->run() == FALSE) {
					...
					$subjects   = $this->subject_m->general_get_order_by_subject_left_examschedule($classesID,$type = 1,$examID,$sectionID); 
					// $subjects   = $this->subject_m->general_get_order_by_subject(array('subject.classesID'=>$classesID)); 
