    ...
    */
    ...
    public function index($maininvoiceclassesID="")
    {
        ...
        if($usertypeID == 3) {
            ...
            // Load only 50 records initially with pagination support
            $this->data['maininvoices']         = $this->maininvoice_m->get_maininvoice_with_studentrelation($schoolyearID, $maininvoiceclassesID, $limit, $offset);
            $this->data['grandtotalandpayment'] = $this->grandtotalandpaid($this->data['maininvoices'], $schoolyearID);
            $this->data['total_records']        = $this->maininvoice_m->count_maininvoice_with_studentrelation($schoolyearID, $maininvoiceclassesID);
            $this->data['loaded_records']       = count($this->data['maininvoices']);
            $this->data['maininvoiceclassesID'] = $maininvoiceclassesID;
    ...
    // AJAX method for lazy loading more invoices
    public function load_more_invoices()
    {
        ...
        $schoolyearID = $this->session->userdata("defaultschoolyearID");
        $feetypes = pluck($this->feetypes_m->get_feetypes(), 'feetypes', 'feetypesID');
        
        // Get more records
        $maininvoices = $this->maininvoice_m->get_maininvoice_with_studentrelation($schoolyearID, $maininvoiceclassesID, $limit, $offset);
        ...
        if(customCompute($maininvoices)) {
            foreach($maininvoices as $maininvoice) {
                ...
                if(isset($grandtotalandpayment['grandtotal'][$maininvoice->maininvoiceID])) { 
                    if(isset($grandtotalandpayment['totalpayment'][$maininvoice->maininvoiceID])) { 
                        if(isset($grandtotalandpayment['totalweaver'][$maininvoice->maininvoiceID])) { 
                            ...
                            $balance = number_format(((float)$grandtotalandpayment['grandtotal'][$maininvoice->maininvoiceID] - (float)$paymentandweaver), 2);
                        } else {
                            $balance = number_format(((float)$grandtotalandpayment['grandtotal'][$maininvoice->maininvoiceID] - (float)$grandtotalandpayment['totalpayment'][$maininvoice->maininvoiceID]), 2);
                        }
                    } else { 
                        if(isset($grandtotalandpayment['totalweaver'][$maininvoice->maininvoiceID])) {
                            $balance = number_format(((float)$grandtotalandpayment['grandtotal'][$maininvoice->maininvoiceID] - (float)$grandtotalandpayment['totalweaver'][$maininvoice->maininvoiceID]), 2);
                        } else {
                            $balance = number_format((float)$grandtotalandpayment['grandtotal'][$maininvoice->maininvoiceID], 2);
                        }
                    }
                } else { 
                    $balance = '0.00';
                ...
                $html .= '<td>'.$maininvoice->srsection.'</td>';
                
                // Fee types
                $html .= '<td>';
                if(isset($grandtotalandpayment['fee_types'][$maininvoice->maininvoiceID])){
                    foreach($grandtotalandpayment['fee_types'][$maininvoice->maininvoiceID] as $key => $feeType){
                        $html .= isset($feetypes[$feeType]) ? $feetypes[$feeType] : '';
                        if(count($grandtotalandpayment['fee_types'][$maininvoice->maininvoiceID])>1){ 
                            $html .= "<br>";
                        }
                    }
                }
                ...
                $html .= '<td>'.number_format($totl, 2).'</td>';
                
                // Discount
                $discount_plus_weaver = 0;
    ...
    public function load_all_invoices()
    {
        $offset = $this->input->post('offset');
        $maininvoiceclassesID = $this->input->post('maininvoiceclassesID');
        ...
        // Get all remaining records (no limit)
        $maininvoices = $this->maininvoice_m->get_maininvoice_with_studentrelation($schoolyearID, $maininvoiceclassesID, NULL, $offset);
        $grandtotalandpayment = $this->grandtotalandpaid($maininvoices, $schoolyearID);
