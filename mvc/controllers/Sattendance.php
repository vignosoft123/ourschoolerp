...
*/
	...
	public function save_attendace()
	{
		...
		if ($_POST) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$isSendVoiceCall = false;
				if ($this->data['setting']->attendance_voice_notification == 'voicecall') {
					$isSendVoiceCall = true;
				}
	...
	public function view($id = null, $url = null)
	{
		...
		if ((int)$id && (int)$url) {
			...
			if (customCompute($this->data["student"]) && customCompute($this->data["classes"])) {
				...
				if ($this->data['setting']->attendance == "subject") { //echo 'if';die;
	...
	// sendVoiceCall
	private function sendVoiceCall($students, $schoolyearID, $classesID, $sectionID)
	{
		// print_r($this->data['setting']);die;
		$attendance_smsgateway = $this->data['setting']->attendance_smsgateway;
		$templateID = $this->data['setting']->attendance_voice_notification_template;
		$mailandsmstemplate = $this->mailandsmstemplate_m->get_mailandsmstemplate($templateID);
		$objStudents = pluck($this->studentrelation_m->get_order_by_student(array('srschoolyearID' => $schoolyearID, 'srclassesID' => $classesID, 'srsectionID' => $sectionID), TRUE), 'obj', 'srstudentID');
		...
		foreach ($students as $student) {
			...
			if(customCompute($user) && $parentsID > 0 && $parentsPhonenumber != '') {
				...
				$message = $mailandsmstemplate->template;
				$this->userConfigTextVoice($message, $user, 3,$schoolyearID);
			}
		}
	}

	//userConfigTextVoice
	private function userConfigTextVoice($message, $user, $usertypeID, $schoolyearID = 1)
	{
		if ($user && $usertypeID) {
			...
			$message = $this->tagConvertor($userTags, $user, $message, 'SMS', $schoolyearID);
			if ($user->phone) {
				$response = $this->send_text_to_voice_call($message, $user->phone);
				return $response;
			} else {
				$send = array('check' => TRUE);
				return $send;
	...
	private function getCallerID()
	{
		...
		return $this->data['caller_list'];
	}

	// send_text_to_voice_call
	private function send_text_to_voice_call($voice_text, $mobile_no)
	{	
		$callerID = $this->getCallerID()[0]->mobile_no;
		$bearer = $this->bearer;
		$curl = curl_init();
		curl_setopt_array($curl, array(
			CURLOPT_URL => "https://voice.vignosoft.com/api_v2/voice-call/campaign",
			CURLOPT_RETURNTRANSFER => true,
			CURLOPT_ENCODING => "",
			CURLOPT_MAXREDIRS => 10,
			CURLOPT_TIMEOUT => 30,
			CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
			CURLOPT_CUSTOMREQUEST => "POST",
			CURLOPT_POSTFIELDS => "caller_id=" . $callerID ."&voice_pitch_rate=medium". "&voice_source=voice_text" . "&voice_text=" . $voice_text . "&mobile_no=" . $mobile_no . "&voice_name=te-IN_0" . "&re_schedule_status=NoAnswer%2CFailed%2CBusy%2CRING_TIMEOUT%2CRejected%2CCompleted&voice_interval=5",
			CURLOPT_HTTPHEADER => array(
		...
		$url = "https://voice.vignosoft.com/api_v2/voice-call/campaign?caller_id=" . $callerID ."&voice_pitch_rate=medium". "&voice_source=voice_text" . "&voice_text=" . $voice_text . "&mobile_no=" . $mobile_no . "&voice_name=te-IN_0" . "&re_schedule_status=NoAnswer%2CFailed%2CBusy%2CRING_TIMEOUT%2CRejected%2CCompleted&voice_interval=5";

		$data = array(
            'request_url' => $url,
            'api_response' => $response,
            'created_on' => date("Y-m-d H:i:s"),
            'type' => "ATTENDANCE VOICE CALL",
        ); 
		 //print_r($resp['error']);
        if ($status == false) {  
	...
	private function sendAbsentWhatsapp($students, $schoolyearID, $classesID, $sectionID)
	{
		...
		// $mailandsmstemplate = $this->mailandsmstemplate_m->get_mailandsmstemplate($templateID);
		 ...
		 $template_sql = "select params,template_name from whatapp_templates where short_name like '%ATTENDANCE%' ";
		$whatsapp_params = $this->db->query($template_sql)->row_array();
		// echo $this->db->last_query();
		// echo "<pre>";print_r( $whatsapp_params);die;
