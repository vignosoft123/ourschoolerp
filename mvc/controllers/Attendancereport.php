		...
		if(permissionChecker('attendancereport')) {
			if($_POST) {
				...
				if ($this->form_validation->run() == FALSE) {
					...
					if($classesID == 0)
					{
					    $students = pluck($this->studentrelation_m->general_get_order_by_student(array('srschoolyearID' => $schoolyearID)), 'obj', 'srstudentID');
					    $sections = pluck($this->section_m->general_get_order_by_section(array()), 'obj', 'sectionID');
					    $attendances = $this->sattendance_m->get_order_by_attendance(array('monthyear' => $monthyear, 'schoolyearID' => $schoolyearID));
					}
					else
					{
					if($sectionID == 0) {
