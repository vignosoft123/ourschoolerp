...
function loadStickyNotes() {
    ...
    $.getJSON('<?=base_url("subdomains/sticky_notes_get")?>', function(res) {
        list.innerHTML = '';
...
function addStickyNote() {
    $.post('<?=base_url("subdomains/sticky_notes_save")?>', { note: '', color: '#fff9c4' }, function(res) {
...
function autoSaveNote(id, ta) {
    ...
    stickyDebounceTimers[id] = setTimeout(function() {
        ...
        $.post('<?=base_url("subdomains/sticky_notes_save")?>', { id: id, note: ta.value, color: color }, function(res) {
...
function changeNoteColor(id, color, el) {
    ...
    $.post('<?=base_url("subdomains/sticky_notes_save")?>', { id: id, note: ta.value, color: color }, null, 'json');
}

function deleteStickyNote(id, btn) {
    if (!confirm('Delete this note?')) return;
    $.post('<?=base_url("subdomains/sticky_notes_delete")?>', { id: id }, function(res) {
        if (res.success) {
