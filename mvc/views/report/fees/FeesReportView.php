        ...
        <div class="rpt-filter-card">
            ...
            <div class="col-sm-12">
                ...
                <?php if($this->session->userdata('usertypeID') == 1) { ?>
                    <div class="form-group col-sm-4" id="touserIDDiv">
                        <label><?= "Users" ?></label>
                        <?php
                        $accountantArray = array(
                            "0" => 'Please Seelct users',
                        );
                        if (customCompute($users)) {
                            foreach ($users as $user) {
                                $accountantArray[$user->userID] = $user->name;
                            }
                        }
                        echo form_dropdown("userID", $accountantArray, set_value("userID"), "id='userID' class='form-control select2'");
                        ?>
                    </div>
                    <?php } ?>
            ...
            <div class="rpt-filter-actions">
                <button id="get_feesreport" class="btn btn-success rpt-filter-btn">
                    <i class="fa fa-search"></i> <?= $this->lang->line("feesreport_submit") ?>
                </button>
            </div>
...
<script type="text/javascript">
    ...
    $(document).on('click', '#get_feesreport', function() {
        $('#load_feesreport').html("");
        var classesID = $('#classesID').val();
        var sectionID = $('#classesID').val() != 0 ? $('#sectionID').val() : 0;
        var studentID = $('#classesID').val() != 0 ? $('#studentID').val() : 0;
