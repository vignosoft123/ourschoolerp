...
.tv-profile-header { background:linear-gradient(135deg,#1a73e8,#0d47a1); padding:22px 16px 16px; text-align:center; }
...
.tv-section-head { background:linear-gradient(90deg,#1a73e8,#1558b0); color:#fff; padding:9px 16px; font-size:12.5px; font-weight:700; text-transform:uppercase; letter-spacing:.5px; display:flex; align-items:center; gap:8px; }
...
.tv-field .tv-val { font-size:13.5px; color:#1a202c; font-weight:500; padding:5px 10px; background:#f8fafc; border-radius:5px; border-left:3px solid #1a73e8; min-height:32px; display:flex; align-items:center; }
.tv-field .tv-val.tv-highlight { border-left-color:#00897b; background:#e6f4f2; color:#00695c; font-weight:700; }
...
<div class="tv-action-bar">
    <div>
        ...
        <?php if(customCompute($tmember) && (($siteinfos->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) && permissionChecker('tmember_edit')): ?>
            <?=btn_sm_edit('tmember/edit/'.$student->studentID."/".$set, $this->lang->line('edit'))?>
        <?php endif; ?>
    ...
    <div class="row">
        ...
        <div class="col-sm-3">
            <div class="tv-profile-card">
                ...
                <ul class="tv-meta-list">
                    <li><b><?=$this->lang->line('tmember_registerNO')?></b> <span><?=$student->srregisterNO?></span></li>
                    <li><b><?=$this->lang->line('tmember_roll')?></b> <span><?=$student->srroll?></span></li>
                    <li><b><?=$this->lang->line('tmember_classes')?></b> <span><?=customCompute($classes) ? $classes->classes : '—'?></span></li>
        ...
        <div class="col-sm-9">
            ...
            <div class="tv-section">
                <div class="tv-section-head"><i class="fa fa-bus"></i> Transport Details</div>
                <div class="tv-section-body">
                    <div class="tv-row cols-4">
                        <div class="tv-field">
                            <label><?=$this->lang->line("tmember_route_name")?></label>
                            <div class="tv-val tv-highlight"><?=customCompute($transport) ? $transport->route : 'N/A'?></div>
                        </div>
                        <div class="tv-field">
                            <label><?=$this->lang->line("tmember_tfee")?></label>
                            <div class="tv-val tv-warn">&#8377; <?=customCompute($tmember) ? number_format($tmember->tbalance, 2) : 'N/A'?></div>
                        </div>
                        <div class="tv-field">
                            <label><?=$this->lang->line("tmember_joindate")?></label>
                            <div class="tv-val"><?php echo (customCompute($tmember) && $tmember->tjoindate) ? date("d M Y", strtotime($tmember->tjoindate)) : 'N/A'; ?></div>
                        </div>
                        <div class="tv-field">
                            <label><?=$this->lang->line("tmember_phone")?></label>
    ...
    <div class="modal fade" id="mail">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header" style="background:linear-gradient(135deg,#1a73e8,#0d47a1);border-radius:5px 5px 0 0;">
...
$("#send_pdf").click(function(){
    ...
    if(error == 0) {
        ...
        $.ajax({
            type:'POST', url:"<?=base_url('tmember/send_mail')?>",
