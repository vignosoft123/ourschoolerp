...
- **2026-08-23**: Second implementation, on a **non-report page**: `mvc/views/student/index.php` (`/student`). Two deviations worth knowing about if reusing this playbook on another non-report page:
  - **Not a report at all** — no `.rpt-action-bar`, no `reports.css` action-bar markup on the page. The Columns button was instead added into the page's own `.student-top-bar` toolbar (styled to match its `.sbar-btn` siblings), while still reusing `.rpt-col-selector-*` from `reports.css` for the dropdown menu itself since that stylesheet is loaded globally via `_layout_main.php` on every page, not just report pages.
  - **Export pattern was 8.6 (server-side PhpSpreadsheet, `Student.php::export_comprehensive_excel()`), but with no real column overlap** — that export dumps 46 fixed columns (bank details, parent Aadhar, etc.) essentially unrelated to the ~13 columns shown on screen. Wiring §5.4's 8.6 branch would have meant guessing a mapping that doesn't really exist. Asked the user; decision was **screen-only** — the Columns dropdown hides/shows the on-screen table only, "Export Excel" is untouched. Don't assume the 8.6 wiring is always the right call — check whether the export's columns actually correspond to the visible table first.
  - **Three near-identical `<table>` markups share `data-col` keys**: the "All Students" tab table, one table per class-section tab (all structurally identical, same permission-gated conditional columns), and an empty-state fallback table. One checkbox list (built from whichever table renders first) toggles `[data-col="x"]` visibility across *all* of them via a plain jQuery class/attribute selector, so hiding a column stays consistent no matter which tab is active.
  - **Used plain `.css('display', ...)` toggling, not a DataTables API**, even though the table has class `dataTable` — DataTables is only lazily initialized on this page (on first status-filter change or tab switch, not on page load), so at load time it's really just a plain HTML table and the manual CSS approach here matches the playbook as originally written. If a future page already has DataTables initialized on load, prefer its native `column(idx).visible()` API instead of manual CSS, since DataTables can redraw/regenerate row nodes and silently undo manual inline styles.
