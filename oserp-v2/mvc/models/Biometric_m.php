...
class Biometric_m extends MY_Model
{

    protected $_table_name     = 'biometric';
    ...
    public function get_biometric_for_report($queryArray)
    {
        // echo 'hhh';die;
        $this->db->select('*');
        $this->db->from('teacher t');
        $this->db->join('biometric b', 't.rfid = b.rfid', 'left');

        // if (isset($queryArray['rfid']) && $queryArray['rfid']) {
        //     $this->db->where('rfid', $queryArray['rfid']);
